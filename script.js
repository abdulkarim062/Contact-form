...
function validate(){
    ...
    if(name==null || name==""){
        showWarning(invalidName, "Enter the Name");
        return false;
    }
    else if(name.length<=2 || name.length>=20){
        showWarning(invalidName, "Name length should be greater than 2 and less than 20");
        return false;
    }
    else if(!isNaN(name)){
    ...
    if(email==null || email==""){
        ...
        showWarning(invalidEmail, "Invalid Email");
    ...
    else if(pw1.length<=4 || pw1.length>=15){
        showWarning(invalidPassword, "Password length should be between 5 and 15");
        return false;
    }
    else{
